...
from config import ASSISTANT_MODEL, BOT_USERNAME, PAGE_SIZE, POLLINATIONS_API_KEY
...
STYLES = """
  ...
  * { box-sizing: border-box; }
  ...
  .shooting-star {
    position: absolute; width: 170px; height: 2.5px; border-radius: 2px;
    background: linear-gradient(90deg, #fff, transparent);
    filter: drop-shadow(0 0 9px #ffffffcc);
...
SCRIPTS = """
  (function () {
    ...
    if (field) {
      var n = window.innerWidth < 640 ? 95 : 190;
      for (var i = 0; i < n; i++) {
        ...
        s.style.setProperty('--min-op', (Math.random() * 0.3 + 0.12).toFixed(2));
        s.style.setProperty('--max-op', (Math.random() * 0.35 + 0.65).toFixed(2));
...
<div id="ai-panel" class="ai-panel">
  <div class="ai-head">
    <div class="ai-title">
      ...
      <div>
        ...
        <div class="ai-sub"><span class="dot"></span>bepul · kalit shart emas</div>
      </div>
    </div>
  ...
  <div class="ai-section active" id="ai-tab-chat">
    ...
    <form id="ai-form" class="ai-foot">
      ...
      <div class="ai-hint">Bepul AI (Pollinations) · hech qanday ro'yxatdan o'tish shart emas</div>
    </form>
  </div>
...
async def _call_pollinations_text(prompt: str, system: str) -> str:
    """Pollinations.ai (bepul, kalitsiz) GET /text so'rovi -- oddiy matn qaytaradi."""
    params = {"model": ASSISTANT_MODEL, "system": system}
    headers = {}
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, params=params, headers=headers,
                ...
                if resp.status != 200:
                    return f"⚠️ AI xatosi ({resp.status}). Birozdan so'ng qayta urinib ko'ring."
                return text.strip() or "..."
    except Exception:
        return "⚠️ AI yordamchiga ulanib bo'lmadi (bepul xizmat band bo'lishi mumkin), birozdan so'ng qayta urinib ko'ring."


async def _call_pollinations_vision(prompt: str, image_data: str, media_type: str) -> str:
    """Pollinations.ai OpenAI-mos /v1/chat/completions orqali rasmni tahlil qiladi.
    Kalitsiz (anonim) urinib ko'radi; agar rad etsa, foydalanuvchiga tushuntiradi."""
    payload = {
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(POLLINATIONS_CHAT_URL, json=payload, headers=headers,
                                     timeout=aiohttp.ClientTimeout(total=45)) as resp:
                if resp.status in (401, 402, 403):
                    return ("⚠️ Rasm tahlili hozircha ishlamadi -- bu funksiya ba'zan bepul "
                            "kalitsiz kirishni cheklaydi. Birozdan so'ng qayta urinib ko'ring, "
                            "yoki bepul https://enter.pollinations.ai kalitini config.py'ga qo'shing.")
...
async def api_assistant(request):
    ...
    if mode == "vision":
        ...
        reply = await _call_pollinations_vision(
            message or "Bu rasmda nima ko'rsatilgan? Batafsil tushuntir.", img_data, media_type
        )
        return web.json_response({"reply": reply})

    if mode == "data":
        csv_text = str(data.get("csv", ""))[:8000]
        if not csv_text:
            return web.json_response({"reply": "CSV fayl topilmadi."}, status=200)
        prompt = f"Quyidagi CSV ma'lumotini tahlil qil.\n\nSo'rov: {message}\n\nCSV:\n{csv_text}"
        reply = await _call_pollinations_text(prompt, DATA_SYSTEM_PROMPT)
        return web.json_response({"reply": reply})
    ...
    for item in raw_history[-8:]:
        if not isinstance(item, dict):
            continue
        role = "Foydalanuvchi" if item.get("role") == "user" else "Sen"
        text = str(item.get("text", ""))[:1500]
        if text:
            transcript_lines.append(f"{role}: {text}")
    transcript_lines.append(f"Foydalanuvchi: {message or 'salom'}")
    prompt = "\n".join(transcript_lines)
    reply = await _call_pollinations_text(prompt, ASSISTANT_SYSTEM_PROMPT)
    return web.json_response({"reply": reply})
